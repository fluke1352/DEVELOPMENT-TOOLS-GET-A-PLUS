...
export default function Chats() {
  ...
  useEffect(
    function socketConnect() {
      const socket = io(
        "https://wannaplay-world-chat.herokuapp.com/world_chat"
      );
      setsocketIO(socket);
      return () => socket.close();
    },
  ...
  useEffect(() => {
    ...
    socketIO.on("has-join-message", (data) =>
      settextArr((prev) => [
        ...prev,
        { message: data.message, time: data.time, socketId: data.socketId },
      ])
    );
    socketIO.on("client-boardcast", (data) =>
      settextArr((prev) => [
        ...prev,
        {
        ...
        >
          {textArr
            ? textArr.map((messages, index) => {
                return (
                  <Box
                    sx={{ mb: 5, flexDirection: "row", display: "flex" }}
                    key={index}
                    style={
                      socketIO.id == messages.socketId
                        ? { color: "#08e8de" }
                        : { color: "#ffffff" }
                    }
                  >
                    <Box sx={{ mr: 5 }}>{messages.time}</Box>
